...
training = pd.read_csv(r"MTG-ML-Conjurer\src\scryfall\PT_train.csv", encoding="utf-8")
...
x = layers.Embedding(500, embed_dim)(encoder_inputs)
_, state_h, state_c = layers.LSTM(latent_dim, return_state=True)(x)
encoder_states = [state_h, state_c]

decoder_inputs = layers.Input(shape=(None,))
x = layers.Embedding(500, embed_dim)(decoder_inputs)
x = layers.LSTM(latent_dim, return_sequences=True)(x, initial_state=encoder_states)
decoder_outputs = layers.Dense(500, activation="softmax")(x)

model = Model([encoder_inputs, decoder_inputs], decoder_outputs)
model.compile(loss="sparse_categorical_crossentropy", optimizer="adam")

# Save the model for use in test_model.py
model.save("PT_model.keras")

with open("PT_vocab.txt", "w", encoding="utf-8") as f:
    for token in vocab:
        if token.strip() != "":
            f.write(token + "\n")
